...
Additional modules: `src/shared/graph.js` is the browser/server graph contract. `src/js/flowLayout.js` uses ELK and allocates a unique port per edge endpoint. `flowPreview.js` owns generated-flow preview controls. `workspace.js` owns local persistence, board actions, search, fit, copy/paste, and selection context. `src/server/usage.js` owns API limits. Browser bundles are generated by `scripts/vendor.js`, not manually edited.
